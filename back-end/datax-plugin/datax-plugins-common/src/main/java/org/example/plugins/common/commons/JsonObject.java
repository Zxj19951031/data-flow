...
package org.example.plugins.common.commons;
...
import org.example.plugins.common.exceptions.IErrorCode;
import org.example.plugins.common.exceptions.JsonError;
...
public class JsonObject implements Serializable {
     ...
     * @return JsonObject
     */
    public static JsonObject newDefault() {
        return JsonObject.from("{}");
    }

    /**
     * 从JSON字符串加载JsonObject
     *
     * @param json 串
     * @return JsonObject
     */
    public static JsonObject from(String json) {
        ...
        try {
            ...
            throw SystemException.newException(JsonError.JSON_PARES_ERROR, e.getMessage());
        }
     ...
     * @return JsonObject
     */
    public static JsonObject from(final Map<String, Object> map) {
        return JsonObject.from(JsonObject.toJSONString(map));
    }
     ...
     * @return JsonObject
     */
    public static JsonObject from(final List<Object> list) {
        return JsonObject.from(JsonObject.toJSONString(list));
    }
     ...
     */
    public String getNecessaryValue(String key, IErrorCode IErrorCode) {
        String value = this.getString(key, null);
        if (StringUtils.isBlank(value)) {
            throw SystemException.newException(IErrorCode,
     ...
     * @throws SystemException 异常
     */
    public Boolean getNecessaryBool(String key, IErrorCode IErrorCode) throws SystemException {
    ...
    public JsonObject getJsonObject(final String path) {
        ...
        return JsonObject.from(JsonObject.toJSONString(object));
    }
    ...
    public Boolean getBool(final String path) {
        ...
        if (null == result) {
            ...
            throw SystemException.newException(JsonError.CONFIG_ERROR,
    ...
    /**
     * 根据用户提供的json path，寻址包含JsonObject的List，如果对象不存在，返回默认null
     *
     * @param path key路径
     * @return list
     */
    public List<JsonObject> getListJsonObject(final String path) {
        ...
        for (final Object object : lists) {
            result.add(JsonObject.from(JsonObject.toJSONString(object)));
        }
        return result;
    }
    ...
    /**
     * 根据用户提供的json path，寻址包含JsonObject的Map，如果对象不存在，返回默认null
     *
     * @param path key 路径
     * @return map
     */
    @SuppressWarnings("unchecked")
    public Map<String, JsonObject> getMapJsonObject(final String path) {
        Map<String, Object> map = this.get(path, Map.class);
        if (null == map) {
            return null;
        }
        ...
        for (final String key : map.keySet()) {
            result.put(key, JsonObject.from(JsonObject.toJSONString(map
                    .get(key))));
        }
     ...
     * 对于插入对象，JsonObject不做任何限制，但是请务必保证该对象是简单对象(包括Map、List)
    ...
    public Object set(final String path, final Object object) {
        ...
        setObject(path, extractJsonObject(object));

        return result;
    }

    /**
     * 获取JsonObject下所有叶子节点的key
     * 对于
     * {"a": {"b": {"c": [0,1,2,3]}}, "x": "y"}
    ...
    /**
     * 合并其他JsonObject，并修改两者冲突的KV配置
     *
     * @param another            合并加入的第三方JsonObject
    ...
    public JsonObject merge(final JsonObject another,
        ...
        for (final String key : keys) {
            ...
            // 使用忽略策略，只有another JsonObject存在但是当前JsonObject不存在的key，才需要更新
    ...
    public String toJSON() {
        return JsonObject.toJSONString(this.getInternal());
    }
    ...
    @SuppressWarnings("unchecked")
    private Object extractJsonObject(final Object object) {
        if (object instanceof JsonObject) {
            return extractFromJsonObject(object);
        }

        if (object instanceof List) {
            List<Object> result = new ArrayList<Object>();
            for (final Object each : (List<Object>) object) {
                result.add(extractFromJsonObject(each));
            }
            return result;
        }
        ...
        if (object instanceof Map) {
            ...
            for (final String key : ((Map<String, Object>) object).keySet()) {
                result.put(key,
                        extractFromJsonObject(((Map<String, Object>) object)
                                .get(key)));
            }
            return result;
        }
    ...
    private Object extractFromJsonObject(final Object object) {
        if (object instanceof JsonObject) {
    ...
    private JsonObject(final String json) {
        try {
    ...
    /**
     * 拷贝当前JsonObject，注意，这里使用了深拷贝，避免冲突
     */
    @Override
    public JsonObject clone() {
        return JsonObject
                .from(JsonObject.toJSONString(this.getInternal()));
    }
}
